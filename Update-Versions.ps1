...
"@
                ...
                # gh pr create --head $BRANCH --title "$( git log --format=%s -1 )" --body "$( git log --format=%b -1 )" --repo "theohbrothers/$( (Get-Item $PSScriptRoot/../../ ).Name )"
                if (!(gh milestone list --state open --query $MILESTONE --json title --jq '.[] | .title')) {
                    gh milestone create --title $MILESTONE
                }
                gh pr create --head $BRANCH --fill --label enhancement --milestone $MILESTONE --repo "$( git remote get-url origin )"
